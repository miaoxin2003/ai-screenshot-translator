...
DEFAULT_SETTINGS = {
    ...
    "custom_models": [],
    "use_streaming": True  # 添加流式输出设置，默认开启
}
...
def analyze_and_translate_image(image, mode="zh-en", callback=None):
...
class SettingsDialog:
    ...
    def __init__(self, parent):
        ...
        self.use_streaming_var = IntVar(value=1 if use_streaming else 0)  # 添加流式输出变量
    ...
    def on_save(self):
        ...
        global api_key, base_url, custom_models, use_streaming
        ...
        new_use_streaming = bool(self.use_streaming_var.get())  # 获取流式输出设置
        ...
        use_streaming = new_use_streaming  # 更新流式输出设置
        api_key = new_api_key
        ...
        settings = {
            ...
            "use_streaming": use_streaming  # 保存流式输出设置
        }
...
class ResultWindow:
    """翻译结果窗口类，分离出来提高响应速度，支持流式更新"""
    def __init__(self, x, y, loading=False):  # 默认不再显示加载提示
        self.x = x
        self.y = y
        self.result = ""
        self.min_width = 350  # 增大初始最小宽度
        self.min_height = 150  # 增大初始最小高度
        self.padding = 20  # 文本周围的内边距
        self.current_text = ""  # 保存当前文本内容
        self.initial_size_set = False  # 标记是否已设置初始大小
    ...
    def setup_ui(self):
        ...
        self.result_text = Text(self.text_frame, wrap=tk.WORD, height=6, width=40)  # 增大初始大小
    ...
    def stream_update(self, text):
        ...
        # 只有当文本变化较大时才调整窗口大小，避免频繁调整
        if not self.initial_size_set or len(text) % 50 == 0:
            if self.resize_timer_id:
                self.window.after_cancel(self.resize_timer_id)
            
            self.resize_timer_id = self.window.after(200, lambda: self.adjust_window_size(text))
            
            # 第一次显示内容时设置为已初始化
            if not self.initial_size_set and text:
    ...
    def adjust_window_size(self, text):
        ...
        # 计算文本区域尺寸，但设置最小值以避免窗口过小
        text_width = max(max_line_length * char_width, 300)
        text_height = max(num_lines * line_height, 120)
        
        # 计算所需的宽度和高度（加上内边距和按钮区域）
        required_width = max(text_width + self.padding * 3, self.min_width)
        required_height = max(text_height + 80, self.min_height)  # 减小按钮区域高度
    ...
    def update_result(self, result):
        ...
        # 调整窗口大小
        self.adjust_window_size(result)
...
class ScreenshotApp:
    ...
    def take_screenshot_and_translate(self):
        ...
        try:
            ...
            # 在截图位置旁边创建结果窗口，不再显示加载信息
            result_window = ResultWindow(mouse_x, mouse_y)
            ...
            def translate_task():
                try:
                    ...
                    def streaming_callback(partial_result):
                        # 在主界面显示结果
                        self.root.after(0, lambda: self.update_streaming_result(partial_result))
                        # 更新结果窗口
                        result_window.stream_update(partial_result)
                    
                    # 调用翻译函数，传入回调函数
                    result = analyze_and_translate_image(screenshot, translation_mode, callback=streaming_callback if use_streaming else None)
    ...
    def update_streaming_result(self, text):
        """更新流式结果到主界面"""
        # 先清空主界面中最后一次结果的内容
        last_pos = self.result_text.search("---", "1.0", tk.END, backwards=True)
        if last_pos:
            # 找到了时间戳行，删除之后的所有内容并重新添加
            self.result_text.delete(last_pos, tk.END)
            timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
            self.result_text.insert(tk.END, f"--- {timestamp} ---\n{text}\n\n")
        else:
            # 如果找不到上一次的结果，直接添加新的结果
            timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
            self.result_text.insert(tk.END, f"--- {timestamp} ---\n{text}\n\n")
            ...
            try:
                ...
                def translate_task():
                    try:
                        ...
                        def streaming_callback(partial_result):
                            # 在主界面显示结果
                            self.root.after(0, lambda: self.update_streaming_area_result(partial_result))
                            # 更新结果窗口
                            result_window.stream_update(partial_result)
    ...
    def update_streaming_area_result(self, text):
        """更新区域截图的流式结果到主界面"""
        # 先清空主界面中最后一次结果的内容
        last_pos = self.result_text.search("---", "1.0", tk.END, backwards=True)
        if last_pos:
